			...
			<div class="px-5">
				<div class="row text-center">
                  ...
                  <h3>Lista de Produtos 
                     <i class="fas fa-plus-circle font-20 color-green pointer ml-2" onClick="window.location='op_produto?op=add_produto';" title="Cadastrar Produto"></i>
                  </h3>
                  <div class="text-center">
                     <form class="form-inline md-form mr-auto mb-4" action="produtos" method="post">
                        <div class="form-group col-md-12 text-center pt-3 pb-2">
                           <input autofocus autocomplete="off" style="text-align:center" class="form-control mr-sm-6" type="text" name="valor_busca_produto" placeholder="Buscar na Lista">
                           <button class="btn btn-primary btn-sm" name="buscar_produto" type="submit"><i class="fas fa-search font-20"></i></button>
                        </div>
                     </form>
                  </div>
                  <table class="table table-striped">
                     <thead>
                        <tr class="table-secondary">
                           <th class="text-center col-sm-1 border" scope="col">Código</th>
                           <th class="text-center col-sm-4 border" scope="col">Produto</th>
                           <th class="text-center col-sm-4 border" scope="col">Descrição</th>
                           <th class="text-center col-sm-2 border" scope="col">Preço</th>
                           <th class="text-center col-sm-1 border" scope="col">Opções</th>
                        </tr>
                     </thead>
                     <tbody>
                        <?php while($item = pg_fetch_assoc($arr_produtos)) { ?>
                           <tr>
                              <th class="text-center middle border" scope="row"><?=$item['id']?></th>
                              <td class="text-center middle border"><?=$item['nome']?></td>
                              <td class="text-center middle border"><?=$item['descricao']?></td>
                              <td class="text-center middle border">R$ <?=number_format($item['preco'],2,",",".")?></td>
                              <td class="text-center middle border">
                                 <i class="fas fa-edit font-20 color-blue pointer" onClick="window.location='op_produto?op=editar_produto&id=<?php echo $item['id'];?>';" title="Editar Produto"></i>
                                 <i class="fas fa-times-circle font-20 color-red pointer" onClick="delete_produto('<?=$item['id']?>'); return false;" title="Remover Produto"></i>
                              </td>
                           </tr>
                        <?php } ?>
                     </tbody>
